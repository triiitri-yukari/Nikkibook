...
class EditImageDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        filename_label = QLabel(f"📁 {self._image.get('original_filename') or self._image['filename']}")
...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        current_mode = self.settings.value("snap_mode", "album")
        ...
        self.area_combo.setStyleSheet(DIALOG_COMBO_STYLE)

        current_area = self.settings.value("snap_capture_area", "full")
        idx_area = self.area_combo.findData(current_area)
        if idx_area >= 0:
            self.area_combo.setCurrentIndex(idx_area)
    ...
    def _retranslate(self):
        ...
        # Rebuild area combo items
        current_area_data = self.area_combo.currentData()
        self.area_combo.blockSignals(True)
        self.area_combo.clear()
        self.area_combo.addItem(t("capture_area_full"), "full")
        self.area_combo.addItem(t("capture_area_mid"), "mid")
        idx_a = self.area_combo.findData(current_area_data)
        if idx_a >= 0:
            self.area_combo.setCurrentIndex(idx_a)
    ...
    def _save_and_accept(self):
        ...
        self.settings.setValue("snap_capture_area", area)
