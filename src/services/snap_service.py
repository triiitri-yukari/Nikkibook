...
from ..config import SNAP_CAPTURE_DEFAULT_OFFSET_X, SNAP_CAPTURE_DEFAULT_SIZE
import pyautogui
...
def run_snap_workflow(
    ...
    capture_width: int = SNAP_CAPTURE_DEFAULT_SIZE[0],
    capture_height: int = SNAP_CAPTURE_DEFAULT_SIZE[1],
    capture_offset_x: int = SNAP_CAPTURE_DEFAULT_OFFSET_X,
) -> Tuple[bytes, str]:
    """
    ...
    try:
        screenshot_bytes = _capture_cropped_screenshot(
            "cropicon.png", step=4,
            width=max(1, int(capture_width)),
            height=max(1, int(capture_height)),
            offset_x=max(0, int(capture_offset_x))
        )
    finally:
        show_ui()
